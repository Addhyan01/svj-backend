...
exports.updateDeliveryStatus = async (req, res) => {
  try {
    ...
    if (status === 'on_the_way') {
      ...
      delivery.deliveredAt = new Date();
      delivery.failReason = null;
      // Jab successfully deliver ho jaye toh structural log 'completed' state me chala jata hai
...
exports.raiseEmergencyRequest = async (req, res) => {
  try {
    ...
    const emergencyLog = await Delivery.create({
      ...
      services: [{ serviceId, quantity: 1 }],   // fixed: use services array per schema
...
exports.raiseEmergencyForMember = async (req, res) => {
  try {
    ...
    if (remainingUnits <= 0) {
      return res.status(400).json({ success: false, message: 'Member ki annual distribution limit exhaust ho chuki hai.' });
    }

    // Deduct 1 unit from subscription quota
    ...
    const ticket = await Delivery.create({
      ...
      deliveryType: 'EMERGENCY',
      status: 'emergency',
      notes: `Emergency request raised by Super Admin on behalf of member ${member.name} (${member.memberId}) via toll-free helpline. 1 subscription unit deducted.`
    });

    return res.status(201).json({
      success: true,
      message: `Emergency ticket raised for ${member.name}. 1 unit deducted from subscription.`,
      data: ticket,
      remainingUnits: membership.totalUnitsEntitled - membership.unitsClaimed
    });
  } catch (error) {
